...
async function getCheeseInfo(link) {
  return axios
    ...
    .then((response) => {
      ...
      const attrib_arr = Array.from(
        document
          .getElementsByClassName("summary-points")[0]
...
async function scrapeByLetter(letter) {
  ...
  // this code works with some corrections, however, it runs *too* fast and gets u blocked by cheese.com :)
  // data = data
  //         .map(c => getCheeseInfo(`https://cheese.com${c.querySelector("a").href}`));
